...
/*** TO BE DONE START ***/
	...
	if(sprintf(message, "%d", msg_no)<0) fail_errno("FAIL to: write msg_no at the beginning of the message buffer");
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &send_time)==-1) fail_errno("FAIL to: Store the current time in send_time");
...
/*** TO BE DONE START ***/
	...
	if(sent_bytes==-1) fail_errno("FAIL to: Send the message through the socket");
...
/*** TO BE DONE START ***/
	...
	recv_bytes = recv(ping_socket, answer_buffer, sent_bytes, MSG_DONTWAIT);
	recv_errno = errno;
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &recv_time)==-1) fail_errno("FAIL to: Store the current time in recv_time");
...
/*** TO BE DONE START ***/
	...
	if(fcntl(ping_socket, F_SETFL, O_NONBLOCK)==-1) fail_errno("FAIL to: change socket behavior to NONBLOCKING");
...
/*** TO BE DONE START ***/
	...
	if(connect(ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen)==-1) fail_errno("FAIL to: connect tcp with the server");
...
/*** TO BE DONE START ***/
	...
	if(ask_socket==-1) fail_errno("FAIL to: create a new TCP socket");
	if(connect(ask_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen)==-1) fail_errno("FAIL to: connect tcp with the server");
...
/** TO BE DONE START ***/
	...
	if(write(ask_socket, request, strlen(request))==-1) fail_errno("FAIL to: Write the request on socket");
