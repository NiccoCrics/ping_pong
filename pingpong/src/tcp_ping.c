...
/*** TO BE DONE START ***/
	...
	if(tcp_socket==-1) fail_errno("FAIL to: create a new TCP socket");
	if(connect(tcp_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen)==-1) fail_errno("FAIL to: connect tcp with the server");
...
/*** TO BE DONE START ***/
	...
	if(write(tcp_socket, request, strlen(request))==-1) fail_errno("FAIL to: Write the request on socket");
